...
*/
double dist_sq_2D(cv::Point2d p1, cv::Point2d p2)
{
...
*/
extern cv::Mat stack_imgs(cv::Mat img1, cv::Mat img2)
{
    cv::Mat stacked = cv::Mat::zeros(MAX(img1.rows, img2.rows), img1.cols + img2.cols, CV_8UC1);

    img1.copyTo(stacked(cv::Rect(0, 0, img1.cols, img1.rows)));
    img2.copyTo(stacked(cv::Rect(img1.cols, 0, img2.cols, img2.rows)));

    // cv::Mat stacked = cv::Mat::zeros(cv::Size(MAX(img1.cols, img2.cols), img1.rows + img2.rows), CV_8UC1);

    // img1.copyTo(stacked(cv::Rect(0, 0, img1.cols, img1.rows)));
    // img2.copyTo(stacked(cv::Rect(0, img1.rows, img2.cols, img2.rows)));
...
*/
void display_big_img(cv::Mat img, char *title)
{
